...
void CodeGen_Hexagon::visit(const Call *op) {
    ...
    if (op->is_intrinsic("gather")) {
        ...
        return;
    } else if (op->is_intrinsic("scatter") || op->is_intrinsic("scatter_acc")) {
        internal_assert(op->args.size() == 4);
        internal_assert(op->type.bits() != 8);
        int index_lanes = op->type.lanes();
        int intrin_lanes = native_vector_bits()/op->type.bits();
        ...
        for (int start = 0; start < index_lanes; start += intrin_lanes) {
            ...
            args.push_back(src_ptr);
            args.push_back(size);
            args.push_back(slice_vector(index, start, intrin_lanes));
            args.push_back(slice_vector(val, start, intrin_lanes));
            value = builder->CreateCall(fn, args);
        }
        return;
    }
